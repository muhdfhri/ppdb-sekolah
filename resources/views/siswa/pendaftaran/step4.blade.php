{{-- resources/views/siswa/pendaftaran/step4.blade.php --}}
@extends('layouts.app')
    ...
    <main class="lg:ml-72 min-h-screen flex flex-col">
        ...
        <header class="hidden lg:flex h-20 bg-white px-8 items-center justify-between" style="border-bottom: 1px solid #f1f5f9;">
            <div class="flex items-center gap-2">
                ...
                <span class="font-semibold text-[#018B3E]">Langkah 4: Pilih Jurusan</span>
            </div>
                ...
                <form action="{{ route('siswa.pendaftaran.step4.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-3 lg:mb-4">
                            Pilihan Jurusan Utama <span class="text-red-500">*</span>
                        </label>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-4">
                            ...
                            <label class="jurusan-label flex items-start gap-3 lg:gap-4 p-4 lg:p-5 rounded-xl border-2 cursor-pointer transition-all"
                                ...
                                <input type="radio" name="jurusan_id" value="{{ $j->id }}" class="sr-only"
                                       {{ $selected ? 'checked' : '' }}
                                       onchange="updateJurusan(this, 'jurusan_id')">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm lg:text-base font-bold text-slate-900 truncate">{{ $j->nama_jurusan }}</p>
                                    <p class="text-[10px] lg:text-xs text-slate-500 mt-0.5">Kuota: {{ $j->kuota }} siswa</p>
                                    @if($j->deskripsi)
                        ...
                        @error('jurusan_id') <p class="text-red-500 text-xs mt-2">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="material-symbols-outlined absolute left-3 lg:left-4 top-1/2 -translate-y-1/2 text-slate-400 text-xl">school</span>
                            <select name="jurusan_id_2"
                                ...
                                <option value="">— Tidak ada pilihan ke-2 —</option>
                                @foreach($jurusan as $j)
                                    <option value="{{ $j->id }}" {{ old('jurusan_id_2', $pendaftaran->jurusan_id_2 ?? '') == $j->id ? 'selected' : '' }}>
                                        {{ $j->kode_jurusan }} - {{ $j->nama_jurusan }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        @error('jurusan_id_2') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
...
function updateJurusan(input, name) {
    document.querySelectorAll(`.jurusan-label`).forEach(label => {
        const radio = label.querySelector(`input[name="${name}"]`);
        if (!radio) return;
        label.style.borderColor = radio.checked ? '#018B3E' : '#f1f5f9';
        label.style.backgroundColor = radio.checked ? 'rgba(1,139,62,0.05)' : '#f8fafc';
    });
}
...
// Set initial jurusan state
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.jurusan-label input:checked').forEach(radio => {
        radio.closest('.jurusan-label').style.borderColor = '#018B3E';
        radio.closest('.jurusan-label').style.backgroundColor = 'rgba(1,139,62,0.05)';
    });
});
</script>
@endsection